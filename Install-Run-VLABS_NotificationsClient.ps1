...
# Auto-generated by Install-Run-VLABS_NotificationsClient.ps1
# DO NOT EDIT MANUALLY - Use the wizard to update configuration
# Version: 0.4.0 - With retry logic, delays, and file logging

$ServerIP = "SERVERIP_PLACEHOLDER"
$ServerPort = SERVERPORT_PLACEHOLDER
$NotifyUri = "http://${ServerIP}:${ServerPort}/api/v1/notify"
$LogPath = "C:\ProgramData\VLABS\Notifications\Logs"
$LogFile = "$LogPath\WSBackup-$(Get-Date -Format 'yyyyMM').log"
...
Write-Log "Triggered by Event ID 14 (Backup Operation Completed)"

# Get the triggering Event 14
$event14 = Get-WinEvent -FilterHashtable @{
...
if (-not $event14) {
    Write-Log "No recent Event 14 found in last 5 minutes - exiting" "WARN"
    exit 0
}

$backupTime = $event14.TimeCreated
Write-Log "Found Event 14 at $($backupTime.ToString('yyyy-MM-dd HH:mm:ss'))"

# RACE CONDITION FIX: Event 4 or 5 may not be logged immediately after Event 14
# Wait and retry logic to ensure we catch the status event
Write-Log "Waiting for Event 4 (Success) or Event 5 (Failure) to be logged..."

$maxRetries = 6  # 6 retries = up to 30 seconds wait time
$retryDelay = 5  # 5 seconds between retries
$successEvent = $null
$failureEvent = $null

for ($i = 1; $i -le $maxRetries; $i++) {
    Write-Log "Attempt $i of $maxRetries - Checking for Event 4/5..."

    # Check for Event ID 4 (successful backup)
    ...
    # Check for Event ID 5 (failed backup)
    ...
    if ($successEvent) {
        Write-Log "Event 4 (Success) found at $($successEvent.TimeCreated.ToString('yyyy-MM-dd HH:mm:ss'))" "SUCCESS"
        break
    } elseif ($failureEvent) {
        Write-Log "Event 5 (Failure) found at $($failureEvent.TimeCreated.ToString('yyyy-MM-dd HH:mm:ss'))" "ERROR"
        break
    } else {
        Write-Log "Event 4/5 not found yet, waiting $retryDelay seconds..." "INFO"
        if ($i -lt $maxRetries) {
...
# Determine backup status and prepare notification
if ($successEvent) {
    ...
    Write-Log "Prepared SUCCESS notification" "SUCCESS"
} elseif ($failureEvent) {
    ...
    Write-Log "Prepared FAILURE notification" "ERROR"
} else {
    # Still no Event 4 or 5 after retries - this is unusual
    Write-Log "Event 4 or 5 not found after $maxRetries retries ($(($maxRetries * $retryDelay)) seconds)" "ERROR"
    Write-Log "This is unusual - Event 14 fired but no status event appeared" "ERROR"
    Write-Log "Exiting without notification to avoid false alerts" "WARN"
    Write-Log "=== Script Ended (No Status Event) ==="
    exit 0
}

# Send notification using transport
Write-Log "Sending notification to NotificationsServer ($NotifyUri)..."
...
try {
    ...
    $response = Invoke-RestMethod -Uri $NotifyUri -Method Post -Body $payload -ContentType "application/json" -TimeoutSec 10 -ErrorAction Stop

    Write-Log "Notification sent successfully!" "SUCCESS"
    Write-Log "Server response: $($response | ConvertTo-Json -Compress)" "SUCCESS"
} catch {
    ...
    # Also try to log to Event Log as backup
...
'@
    ...
    # Create action (use full path to PowerShell.exe for reliability when running as SYSTEM)
